...
<div id="main" class="wrap">
	...
	<div class="main">
		<h2>修改商品</h2>
		<div class="manage">
			<form action="productviewservlet" method="post" id="regForm" enctype="multipart/form-data">
				<input type="hidden" value="5" name="option">
				<input type="hidden" value="${modifyproduct.ep_id }" name="moproduct">
				<table class="form">
					<tr>
						<td class="field">商品名称(*)：</td>
						<td><input type="text" class="text" name="productName" value="${modifyproduct.ep_name }" /><span></span></td>
					</tr>
                    <tr>
						<td class="field">描述：</td>
						<td><input type="text" class="text" name="productDs" /></td>
					</tr>
                    <tr>
						<td class="field">品牌：</td>
						<td><input type="text" class="text" name="productBrand" /></td>
					</tr>
					<tr>
						<td class="field">所属父类：</td>
						<td>
							<select name="parentCategory">
								<c:forEach var="category" items="${categories }">
									<c:if test="${category.epc_parent_id==0 }">
										<option value="${category.epc_name }">${category.epc_name }</option>
									</c:if>
								</c:forEach>
							</select>
						所属子类：
					...
					<tr>
						...
						<td><input type="text" class="text tiny" name="productPrice" value="${modifyproduct.ep_price }" /> 元<span></span></td>
					</tr>
					
					<tr>
						<td class="field">商品原价(选填)：</td>
						<td><input type="text" class="text tiny" name="productOriginPrice" value="${modifyproduct.ep_origin_price }" /> 元<span></span></td>
					</tr>
					
					<tr>
						<td class="field">库存(*)：</td>
						<td><input type="text" class="text tiny" name="productNumber" value="${modifyproduct.ep_stock }"/><span></span></td>
					</tr>
					<tr>
