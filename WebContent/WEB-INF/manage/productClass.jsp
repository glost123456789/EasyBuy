...
<div id="main" class="wrap">
	...
	<div class="main">
		...
		<div class="manage">
			<table class="list">
				...
				<c:set var="count" scope="page" value="1"></c:set>
				<c:forEach var="category" items="${showcategories }" varStatus="status">
						<c:choose>
							<c:when test="${category.epc_parent_id==0 }">
								<tr>
									<td class="first w4 c">${status.index }</td>
									<td>${category.epc_name }</td>
									<td class="w1 c"><a href="categoryservlet?option=5&id=${category.epc_id }">修改</a> <a class="manageDel" href="categoryservlet?option=5&id=${category.epc_id }">删除</a></td>
								</tr>
							</c:when>
							<c:otherwise>
								<tr>
									<td class="first w4 c">${status.index }</td>
									<td class="childClass">${category.epc_name }</td>
									<td class="w1 c"><a href="categoryservlet?option=5&id=${category.epc_id }">修改</a> <a class="manageDel" href="categoryservlet?option=5&id=${category.epc_id }">删除</a></td>
								</tr>
							</c:otherwise>
						</c:choose>
    ...
    <div class="pager">
<ul class="clearfix">
					<li><a href="categoryservlet?option=1&currentpage=1">首页</a></li>
					<li>...</li>
					<c:choose>
						...
						<c:forEach begin="1" end="${totalpage }" var="pageNumber">
							<c:choose>
								...
								<c:otherwise>
									<li><a href="categoryservlet?option=1&currentpage=${pageNumber }">${pageNumber }</a></li>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						</c:when>
                    ...
                    <li>...</li>
					<li><a href="categoryservlet?option=1&currentpage=${totalpage }">尾页</a></li>
				</ul>
			</div>
</div>
<div id="footer">
