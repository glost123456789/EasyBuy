...
<div id="main" class="wrap">
	...
	<div class="main">
		...
		<div class="manage">
			<table class="list">
				...
				<c:forEach var="manageproduct" items="${pdtlist }" varStatus="status">
					<tr>
						<td class="first w4 c">${status.index+1 }</td>
						<td class="thumb"><img src="files/${manageproduct.ep_file_name }" /><a href="productviewservlet?option=1&productname=${manageproduct.ep_id }" target="_self">${manageproduct.ep_name }</a></td>
						<td class="w1 c"><a href="productviewservlet?option=4&productname=${manageproduct.ep_id }">修改</a> <a class="manageDel" href="productviewservlet?option=3&productname=${manageproduct.ep_id }">删除</a></td>
					</tr>
				</c:forEach>
			</table>
		</div>
    ...
    <div class="pager">
				<ul class="clearfix">
<li><a href="productviewservlet?option=2&currentpage=1">首页</a></li>
					<li>...</li>
					<c:choose>
						...
						<c:forEach begin="1" end="${totalpage }" var="pageNumber">
							<c:choose>
								...
								<c:otherwise>
									<li><a href="productviewservlet?option=2&currentpage=${pageNumber }">${pageNumber }</a></li>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						</c:when>
					...
					<c:if test="${currentpage-3<=0 }">
						<c:forEach begin="1" end="6" var="pageNumber">
							<c:choose>
								...
								<c:otherwise>
									<li><a href="oproductviewservlet?option=2&currentpage=${pageNumber }">${pageNumber }</a></li>
								</c:otherwise>
							</c:choose>
						</c:forEach>
					</c:if>
                    ...
                    <li>...</li>
					<li><a href="productviewservlet?option=2&currentpage=${totalpage }">尾页</a></li>
				</ul>
			</div>
</div>
<div id="footer">
