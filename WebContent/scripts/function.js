...
*/
//ajax设置用户地址
function AddandSetAddress(address){
	$.ajax({
		"url":"shoppingservlet?option=7",
		"data":{"adaddress":address},
		"method":"POST",
...
//异步修改订单状态
function changeorderstatus(obj,idstring){
	$.ajax({
		"url":"orderservlet?option=3",
		"data":{"status":obj.value,"orderid":idstring,"random":Math.random()},
		"success":function(data){
			$(obj).prev().html(data);
			if(obj.value==5)
				{
...
function goBuy(id, price){
    ...
    if(oldCookie) {
        newCookie=newString(oldCookie.split(","), id) ;
    } else {
        newCookie = id;
    }
...
$(function(){
    ...
    $("#changeCode").click(function(){
        $("#safeCode").attr("src","verifycodeservlet?id="+Math.random());
    });
    //注册页面验证
...
//            location.href = "Cart?action=addAddress&address="+$("#addAddr").val();
    ...
    $(".delete").find("a").click(function(){
        ...
        var $idd=$($tds[1]);
        var shopid=$idd.find("input[name='id']").val();
        if(confirm("确定要删除吗？")) {
            var oldCookie = getCookie("product");
            if(oldCookie) {
    ...
    $(".number").find("span").click(function(){
        ...
        var $price= $($tds[1]);
        var $number=$($tds[2]);
        var price = $price.find("input[name='price']").val();//存入价格
        var thisid=$price.find("input[name='id']").val();//存入id
        ...
        $.ajax({
        	"url":"shoppingservlet?option=2",
        	"data":{"id":thisid,"count":number,"price":price*number,"random":Math.random()},
        	"success":function(data){
        	},
        });
    });
    ...
    $("#logout").click(function(){
        if(confirm("购物车中尚有未结算的商品，是否结账？")) {
            ...
            location.href="easybuyuserservlet?option=5";
            return false;
        }
    });
    ...
    $("#shoppingBag").mouseover(function(){
    	...
    	$.ajax({
    		  ...
    		  "success":function(data){
    			  ...
    			  {
    				  var aaa=a[i].split("b%b");
    				  shopbus+="<tr><th><img width='20px' height='20px' src='files/"+aaa[0]+"'></img></th><th>"+aaa[1]+"</th><th>"+aaa[2]+"</th></tr>"; 
    			      $table.html(shopbus);
    			      $("#floatBag").show();
    			  }
    		  }
